...
#Define positions
central_midfield_positions = ['Centre Attacking Midfielder', 'Centre Defensive Midfielder',  
                              'Right Centre Midfielder',  'Left Centre Midfielder']
df = df[(df['primary_position'].isin(central_midfield_positions)) | (df['secondary_position'].isin(central_midfield_positions))].reset_index(drop = True)
...
df['Deep_lying_playmaker_rating'] = df['Progressive_passes'] * 0.2 \
                               + df['Chance_creation'] * 0.2 \
                               + df['Assists'] * 0.1 \
                               + df['Set_pieces'] * 0.15 \
                               + df['Durability'] * 0.1 \
                               + df['Distribution'] * 0.25 

df['Box_to_box_rating'] = df['Chance_creation'] * 0.1 \
                               + df['Regains'] * 0.15 \
                               + df['Progressive_runs'] * 0.15 \
                               + df['Heading'] * 0.1 \
                               + df['Durability'] * 0.15 \
                               + df['Defending'] * 0.1 \
                               + df['Positioning'] * 0.15 \
...
df['Central_midfielder_general_rating'] = df['Progressive_passes'] * 0.2 \
                               + df['Assists'] * 0.1 \
                               + df['Durability'] * 0.2 \
...
df['Deep_lying_playmaker_rating'] = np.round(minmax_norm(df['Deep_lying_playmaker_rating']), 0)
df['Box_to_box_rating'] = np.round(minmax_norm(df['Box_to_box_rating']), 0)
df['Central_midfielder_general_rating'] = np.round(minmax_norm(df['Central_midfielder_general_rating']), 0)                                       
                       
#Profiling
df['avg_rating'] = np.round(df[['Deep_lying_playmaker_rating', 'Box_to_box_rating', 'Central_midfielder_general_rating']].mean(axis = 1), 2)
Profiling = df[['player_name','team_name','Deep_lying_playmaker_rating', 'Box_to_box_rating', 'Central_midfielder_general_rating', 'avg_rating', 'primary_position','secondary_position','player_season_minutes']].sort_values('avg_rating', ascending = False)
Profiling.head(20)
Profiling.to_csv('USL_2022_Central_midfielders_profiling.csv')

#Descriptors
df = add_descriptor_stats(df)
Descriptors = df[['player_name','team_name','Regains','Heading','Durability','Defending','Positioning','Passing', 'Progressive_passes', 'Distribution', 'Chance_creation', 'Assists', 'Set_pieces', 'Progressive_runs', 'player_season_right_foot_ratio', 'weak_foot_ratio']]
Descriptors.head(20)
Descriptors.to_csv('USL_2022_Central__midfielders_descriptors.csv')
